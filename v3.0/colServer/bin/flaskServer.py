...
def createVM():
	...
	if len(machineDetails)==5:
		...
		print imagePath
		print "-----------------"
		cmd=str("scp ")+str(imageDetails[1].strip('\n\r'))+str(" ")+str("~/")+str(imageName)+str(clientCount)+str(".")+str(imageExt)
		print cmd
		cmd=str("scp ")+str("~/")+str(imageName)+str(clientCount)+str(".")+str(imageExt)+str(" ")+str(machineDetails[1])+str(":/")+str(imageName)+str(clientCount)+str(".")+str(imageExt)
		print cmd
		print type(cmd)
...
#		return jsonify(vmInfo)
    	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(vmInfo)+')'
		print ast
		return ast
	return jsonify(vmInfo)
...
def queryVM():	
	...
	if clientDetails:
		vmQuery={'vmid':vmid,'name':clientDetails[2],'instance_type':clientDetails[3],'pmid':clientDetails[4]}
	else:
		vmQuery={'vmid':0,'name':None,'instance_type':0,'pmid':0}
    	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(vmQuery)+')'
		print ast
		return ast
	return jsonify(vmQuery)
...
def listTypes():
    	jsonpFunc = str(request.args.get('jsonp_callback'))
    	vmType=json.loads(open(sys.argv[3],'r').read())
    	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(vmType)+')'
		print ast
		return ast
...
def listImages():
	...
	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(imagesList)+')'
		print ast
		return ast
...
def queryVolume():
	...
	if volumeDetails:
		...
		if status=="attached":
			volumeQuery={'volumeid':volumeid,'name':name,'size':size,'status':status,'vmid':vmid}
		elif status=="available":
			volumeQuery={'volumeid':volumeid,'name':name,'size':size,'status':status}
		else:
			volumeQuery={'error':"volumeid: "+volumeid+" does not exist"}
	else:
		volumeQuery={'error':"volumeid: "+str(volumeid)+" does not exist"}
    	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(volumeQuery)+')'
		print ast
		return ast
	return jsonify(volumeQuery);
...
def detachVolume():
	...
	if volumeDetails:
		...
		detach={'status':0}
	if jsonpFunc!='None':
		ast=jsonpFunc+'('+json.dumps(detach)+')'
		print ast
		return ast
	return jsonify(detach);
